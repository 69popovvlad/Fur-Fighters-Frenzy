...
namespace Client.GameLogic.Inputs.Parts
{
    public class DodgeInputPart : IInputPart
    {
        ...
        public void Update(in InputPartData data, float delta)
        {
            ...
            if (Math.Abs(x) < movementTolerance && Math.Abs(z) < movementTolerance || !Input.GetKeyDown(KeyCode.Space))
            {
                return;
            }

            var command = new DodgeInputCommand(data.OwnerKey, x, z);
